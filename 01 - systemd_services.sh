...
check_debian() {
    if ! grep -qi "debian" /etc/os-release 2>/dev/null; then
        log_warn "Distribution non détectée comme Debian. Poursuite quand même."
    fi
}
...
setup_firewalld() {
    ...
    if dpkg -s ufw &>/dev/null && systemctl is-active --quiet ufw 2>/dev/null; then
        log_warn "ufw est actif et entre en conflit avec firewalld."
