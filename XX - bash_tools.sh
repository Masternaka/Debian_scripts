...
# ---- Liste des outils (dépôts officiels Debian) ----
# Note : fd-find fournit la commande fdfind, nous créerons un alias fd.
#        starship n'est pas dans les dépôts, il devra être installé manuellement.
PACKAGES=(
    bat             # cat avec coloration syntaxique
    eza             # ls moderne (fork actif de exa)
    ripgrep         # grep ultra-rapide (rg)
    fd-find         # find simplifié (commande fdfind)
    fzf             # fuzzy finder
...
echo -e "${CYAN}>>> Mise à jour de l'index des paquets...${NC}"
run apt-get update
...
if ! command -v starship &> /dev/null; then
    echo -e "${YELLOW}⚠️  starship n'est pas installé. Il n'est pas disponible dans les dépôts Debian.${NC}"
    echo -e "   Pour l'installer, exécutez : curl -sS https://starship.rs/install.sh | sh"
...
# fd-find installé sous le nom fdfind
alias fd='fdfind'
alias find='fd'
alias df='duf'
